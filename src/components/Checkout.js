...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    ...
    try {
      // TODO: CHECKOUT - Add new address to the backend and display the latest list of addresses
  ...
  const deleteAddress = async (token, addressId) => {
    ...
    try {
      // TODO: CHECKOUT - Delete selected address from the backend and display the latest list of addresses
  ...
  // TODO: CHECKOUT - Validate request for checkout
  /**
  ...
  const validateRequest = (items, addresses) => {
    ...
    let cartPrice = getTotalCartValue(items);
    if (Number(balance) < cartPrice) {
      enqueueSnackbar(
        "You do not have enough balance in your wallet for this purchase",
        {
          variant: "warning",
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      ...
      // if cart has been succesfully placed
  ...
  // TODO: CHECKOUT - Fetch addressses if logged in, otherwise show info message and redirect to Products page
      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Divider />
            <Box>
              {/* TODO: CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
...

              {
                addresses.all.map((addressObj) => {
                  let className = "address-item not-selected";

                  if (addressObj["_id"] === addresses.selected) {
                  ...
                  return (
                    <div
                      className={className}
                      key={addressObj["_id"]}
                      onClick={() => { selectAddress(addressObj["_id"]) }}
                    >
                      <Typography my="1rem">
                        {addressObj.address}
                      </Typography>
            ...
            {/* TODO: CHECKOUT - Dislay either "Add new address" button or the <AddNewAddressView> component to edit the currently selected address */}
